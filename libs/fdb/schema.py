...
TRIGGER_DDL_TYPES = [
    ...
    None,
    None,
    None,  # gap for TRIGGER_TYPE_MASK - 3 bits
    "CREATE EXCEPTION",
    "ALTER EXCEPTION",
    "DROP EXCEPTION",
    "CREATE VIEW",
...
def get_grants(privileges, grantors=None):
    ...
    for k, g in groupby(p, gkey):
        ...
        item = g[0]
        if item.has_grant():
            admin_option = " WITH %s OPTION" % (
                "ADMIN" if item.privilege == "M" else "GRANT"
            )
        else:
            admin_option = ""
        ...
        for k, items in groupby(g, gkey2):
            ...
            if item.privilege in tp:
                ...
                if len(items) > 1:
                    privilege += "(%s)" % ",".join(
                        i.field_name for i in items if i.field_name
                    )
        ...
        if priv_list:
            ...
            privilege += " ON "
        grants.append(
            "GRANT %s%s TO %s%s%s%s"
            % (privilege, sname, utype, uname, admin_option, granted_by)
        )
    return grants
...
class Schema(Visitable):
    ...
    def _get_field_dimensions(self, field):
        return [
            (r[0], r[1])
            for r in self._ic.execute(
                """SELECT r.RDB$LOWER_BOUND, r.RDB$UPPER_BOUND
FROM RDB$FIELD_DIMENSIONS r
where r.RDB$FIELD_NAME = '%s'
order by r.RDB$DIMENSION"""
                % field.name
            )
        ]
    ...
    def _get_collations(self):
        if self.__collations is None:
            ...
            self.__collations = ObjectList(
                (Collation(self, row) for row in self._ic.itermap()),
                Collation,
                "item.name",
            )
    ...
    def _get_character_sets(self):
        if self.__character_sets is None:
            ...
            self.__character_sets = ObjectList(
                (CharacterSet(self, row) for row in self._ic.itermap()),
                CharacterSet,
                "item.name",
            )
    ...
    def _get_exceptions(self):
        if self.__exceptions is None:
            ...
            self.__exceptions = ObjectList(
                (DatabaseException(self, row) for row in self._ic.itermap()),
                DatabaseException,
                "item.name",
            )
    ...
    def _get_all_domains(self):
        if self.__domains is None:
            ...
            sys_domains, user_domains = ObjectList(
                (Domain(self, row) for row in self._ic.itermap()), Domain, "item.name"
            ).split(lambda item: item.issystemobject())
            sys_domains.freeze()
            user_domains.freeze()
    ...
    def _get_all_tables(self):
        if self.__tables is None:
            ...
            sys_tables, user_tables = ObjectList(
                (Table(self, row) for row in self._ic.itermap()), Table, "item.name"
            ).split(lambda item: item.issystemobject())
            sys_tables.freeze()
            user_tables.freeze()
    ...
    def _get_all_views(self):
        if self.__views is None:
            ...
            sys_views, user_views = ObjectList(
                (View(self, row) for row in self._ic.itermap()), View, "item.name"
            ).split(lambda item: item.issystemobject())
            sys_views.freeze()
            user_views.freeze()
...
from RDB$RELATION_CONSTRAINTS where RDB$INDEX_NAME is not null"""
            ...
            self.__constraint_indices = dict(
                (key.strip(), value.strip()) for key, value in self._ic
            )
...
RDB$EXPRESSION_SOURCE, RDB$STATISTICS from RDB$INDICES"""
            ...
            sys_indices, user_indices = ObjectList(
                (Index(self, row) for row in self._ic.itermap()), Index, "item.name"
            ).split(lambda item: item.issystemobject())
            sys_indices.freeze()
            user_indices.freeze()
    ...
    def _get_all_generators(self):
        if self.__generators is None:
            ...
            sys_generators, user_generators = ObjectList(
                (Sequence(self, row) for row in self._ic.itermap()),
                Sequence,
                "item.name",
            ).split(lambda item: item.issystemobject())
    ...
    def _get_all_triggers(self):
        if self.__triggers is None:
            ...
            sys_triggers, user_triggers = ObjectList(
                (Trigger(self, row) for row in self._ic.itermap()), Trigger, "item.name"
            ).split(lambda item: item.issystemobject())
            sys_triggers.freeze()
    ...
    def _get_all_procedures(self):
        if self.__procedures is None:
            ...
            sys_procedures, user_procedures = ObjectList(
                (Procedure(self, row) for row in self._ic.itermap()),
                Procedure,
                "item.name",
            ).split(lambda item: item.issystemobject())
...
and (c.RDB$CONSTRAINT_TYPE in ('CHECK','NOT NULL'))"""
            ...
            self.__constraints = ObjectList(
                (Constraint(self, row) for row in self._ic.itermap()),
                Constraint,
                "item.name",
            )
    ...
    def _get_roles(self):
        if self.__roles is None:
            ...
            self.__roles = ObjectList(
                (Role(self, row) for row in self._ic.itermap()), Role, "item.name"
            )
            self.__roles.freeze()
        return self.__roles
    ...
    def _get_dependencies(self):
        if self.__dependencies is None:
            ...
            self.__dependencies = ObjectList(
                (Dependency(self, row) for row in self._ic.itermap()), Dependency
            )
    ...
    def _get_all_functions(self):
        if self.__functions is None:
            ...
            sys_functions, user_functions = ObjectList(
                (Function(self, row) for row in self._ic.itermap()),
                Function,
                "item.name",
            ).split(lambda item: item.issystemobject())
...
order by RDB$FILE_SEQUENCE"""
            ...
            self.__files = ObjectList(
                (DatabaseFile(self, row) for row in self._ic.itermap()),
                DatabaseFile,
                "item.name",
            )
            self.__files.freeze()
    ...
    def _get_shadows(self):
        if self.__shadows is None:
            self.__fail_if_closed()
            self._ic.execute(
                """select RDB$FILE_FLAGS, RDB$SHADOW_NUMBER
from RDB$FILES
where RDB$SHADOW_NUMBER > 0 AND RDB$FILE_SEQUENCE = 0
order by RDB$SHADOW_NUMBER"""
            )
            self.__shadows = ObjectList(
                (Shadow(self, row) for row in self._ic.itermap()), Shadow, "item.name"
            )
...
FROM RDB$USER_PRIVILEGES"""
    ...
    def _get_backup_history(self):
        if self.__backup_history is None:
            self.__fail_if_closed()
            self._ic.execute(
                """SELECT RDB$BACKUP_ID, RDB$TIMESTAMP,
RDB$BACKUP_LEVEL, RDB$GUID, RDB$SCN, RDB$FILE_NAME
FROM RDB$BACKUP_HISTORY"""
            )
            self.__backup_history = ObjectList(
                (BackupHistory(self, row) for row in self._ic.itermap()),
                BackupHistory,
                "item.name",
            )
    ...
    def _get_filters(self):
        if self.__filters is None:
            self.__fail_if_closed()
            self._ic.execute(
                """SELECT RDB$FUNCTION_NAME, RDB$DESCRIPTION,
RDB$MODULE_NAME, RDB$ENTRYPOINT, RDB$INPUT_SUB_TYPE, RDB$OUTPUT_SUB_TYPE, RDB$SYSTEM_FLAG
FROM RDB$FILTERS"""
            )
            self.__filters = ObjectList(
                (Filter(self, row) for row in self._ic.itermap()), Filter, "item.name"
            )
    ...
    def _get_users(self):
        if self.__users is None:
            ...
            self.__users = ObjectList(
                (fdb.services.User(row[0].strip()) for row in self._ic),
                fdb.services.User,
                "item.name",
            )
        return self.__users
    ...
    def _get_packages(self):
        if self.__packages is None:
            if self._con.ods >= fdb.ODS_FB_30:
                self.__fail_if_closed()
                self._ic.execute(
                    """select RDB$PACKAGE_NAME, RDB$PACKAGE_HEADER_SOURCE,
    RDB$PACKAGE_BODY_SOURCE, RDB$VALID_BODY_FLAG, RDB$SECURITY_CLASS, RDB$OWNER_NAME,
    RDB$SYSTEM_FLAG, RDB$DESCRIPTION
                FROM RDB$PACKAGES"""
                )
                self.__packages = ObjectList(
                    (Package(self, row) for row in self._ic.itermap()),
                    Package,
                    "item.name",
                )
            else:
    ...
    description = property(
        lambda self: self.__description,
        doc="Database description or None if it doesn't have a description.",
    )
    owner_name = property(lambda self: self.__owner, doc="Database owner name.")
    default_character_set = LateBindingProperty(
        _get_default_character_set, doc="Default :class:`CharacterSet` for database"
    )
    security_class = property(
        lambda self: self.__security_class,
        doc="Can refer to the security class applied as databasewide access control limits.",
    )
    collations = LateBindingProperty(
        _get_collations,
        doc=":class:`~fdb.utils.ObjectList` of all collations in database.\nItems are :class:`Collation` objects.",
    )
    character_sets = LateBindingProperty(
        _get_character_sets,
        doc=":class:`~fdb.utils.ObjectList` of all character sets in database.\nItems are :class:`CharacterSet` objects.",
    )
    exceptions = LateBindingProperty(
        _get_exceptions,
        doc=":class:`~fdb.utils.ObjectList` of all exceptions in database.\nItems are :class:`DatabaseException` objects.",
    )
    generators = LateBindingProperty(
        _get_generators,
        doc=":class:`~fdb.utils.ObjectList` of all user generators in database.\nItems are :class:`Sequence` objects.",
    )
    sysgenerators = LateBindingProperty(
        _get_sysgenerators,
        doc=":class:`~fdb.utils.ObjectList` of all system generators in database.\nItems are :class:`Sequence` objects.",
    )
    sequences = LateBindingProperty(
        _get_generators,
        doc=":class:`~fdb.utils.ObjectList` of all user generators in database.\nItems are :class:`Sequence` objects.",
    )
    syssequences = LateBindingProperty(
        _get_sysgenerators,
        doc=":class:`~fdb.utils.ObjectList` of all system generators in database.\nItems are :class:`Sequence` objects.",
    )
    domains = LateBindingProperty(
        _get_domains,
        doc=":class:`~fdb.utils.ObjectList` of all user domains in database.\nItems are :class:`Domain` objects.",
    )
    sysdomains = LateBindingProperty(
        _get_sysdomains,
        doc=":class:`~fdb.utils.ObjectList` of all system domains in database.\nItems are :class:`Domain` objects.",
    )
    indices = LateBindingProperty(
        _get_indices,
        doc=":class:`~fdb.utils.ObjectList` of all user indices in database.\nItems are :class:`Index` objects.",
    )
    sysindices = LateBindingProperty(
        _get_sysindices,
        doc=":class:`~fdb.utils.ObjectList` of all system indices in database.\nItems are :class:`Index` objects.",
    )
    tables = LateBindingProperty(
        _get_tables,
        doc=":class:`~fdb.utils.ObjectList` of all user tables in database.\nItems are :class:`Table` objects.",
    )
    systables = LateBindingProperty(
        _get_systables,
        doc=":class:`~fdb.utils.ObjectList` of all system tables in database.\nItems are :class:`Table` objects.",
    )
    views = LateBindingProperty(
        _get_views,
        doc=":class:`~fdb.utils.ObjectList` of all user views in database.\nItems are :class:`View` objects.",
    )
    sysviews = LateBindingProperty(
        _get_sysviews,
        doc=":class:`~fdb.utils.ObjectList` of all system views in database.\nItems are :class:`View` objects.",
    )
    triggers = LateBindingProperty(
        _get_triggers,
        doc=":class:`~fdb.utils.ObjectList` of all user triggers in database.\nItems are :class:`Trigger` objects.",
    )
    systriggers = LateBindingProperty(
        _get_systriggers,
        doc=":class:`~fdb.utils.ObjectList` of all system triggers in database.\nItems are :class:`Trigger` objects.",
    )
    procedures = LateBindingProperty(
        _get_procedures,
        doc=":class:`~fdb.utils.ObjectList` of all user procedures in database.\nItems are :class:`Procedure` objects.",
    )
    sysprocedures = LateBindingProperty(
        _get_sysprocedures,
        doc=":class:`~fdb.utils.ObjectList` of all system procedures in database.\nItems are :class:`Procedure` objects.",
    )
    constraints = LateBindingProperty(
        _get_constraints,
        doc=":class:`~fdb.utils.ObjectList` of all constraints in database.\nItems are :class:`Constraint` objects.",
    )
    roles = LateBindingProperty(
        _get_roles,
        doc=":class:`~fdb.utils.ObjectList` of all roles in database.\nItems are :class:`Role` objects.",
    )
    dependencies = LateBindingProperty(
        _get_dependencies,
        doc=":class:`~fdb.utils.ObjectList` of all dependencies in database.\nItems are :class:`Dependency` objects.",
    )
    functions = LateBindingProperty(
        _get_functions,
        doc=":class:`~fdb.utils.ObjectList` of all user functions defined in database.\nItems are :class:`Function` objects.",
    )
    sysfunctions = LateBindingProperty(
        _get_sysfunctions,
        doc=":class:`~fdb.utils.ObjectList` of all system functions defined in database.\nItems are :class:`Function` objects.",
    )
    files = LateBindingProperty(
        _get_files,
        doc=":class:`~fdb.utils.ObjectList` of all extension files defined for database.\nItems are :class:`DatabaseFile` objects.",
    )
    shadows = LateBindingProperty(
        _get_shadows,
        doc=":class:`~fdb.utils.ObjectList` of all shadows defined for database.\nItems are :class:`Shadow` objects.",
    )
    privileges = LateBindingProperty(
        _get_privileges,
        doc=":class:`~fdb.utils.ObjectList` of all privileges defined for database.\nItems are :class:`Privilege` objects.",
    )
    backup_history = LateBindingProperty(
        _get_backup_history,
        doc=":class:`~fdb.utils.ObjectList` of all nbackup hisotry records.\nItems are :class:`BackupHistory` objects.",
    )
    filters = LateBindingProperty(
        _get_filters,
        doc=":class:`~fdb.utils.ObjectList` of all user-defined BLOB filters.\nItems are :class:`Filter` objects.",
    )
    # FB 3
    packages = LateBindingProperty(
        _get_packages,
        doc=":class:`~fdb.utils.ObjectList` of all packages defined for database.\nItems are :class:`Package` objects.",
    )
    ...
    def bind(self, connection):
        ...
        enum_select = (
            "select RDB$TYPE, RDB$TYPE_NAME from RDB$TYPES where RDB$FIELD_NAME = ?"
        )

        def enum_dict(enum_type):
            return dict(
                (key, value.strip())
                for key, value in self._ic.execute(enum_select, (enum_type,))
            )

        # Object types
        self.enum_object_types = enum_dict("RDB$OBJECT_TYPE")
        # Object type codes
        self.enum_object_type_codes = dict(
            ((value, key) for key, value in self.enum_object_types.items())
        )
        # Character set names
    ...
    def get_metadata_ddl(self, sections=SCRIPT_DEFAULT_ORDER):
        ...
        def order_by_dependency(items, get_dependencies):
            ...
            while len(wlist) > 0:
                ...
                for dep in get_dependencies(item):
                    if (
                        isinstance(dep.depended_on, View)
                        and dep.depended_on not in ordered
                    ):
                        wlist.append(item)
                        add = False
                        break
        ...
        def view_dependencies(item):
            return [x for x in item.get_dependencies() if x.depended_on_type == 1]
            ...
            if section == SCRIPT_COLLATIONS:
                ...
                for charset in self.character_sets:
                    if charset.name != charset.default_collate.name:
                        script.append(
                            charset.get_sql_for(
                                "alter", collation=charset.default_collate.name
                            )
                        )
                ...
                for func in (
                    ...
                    if not x.isexternal() and not x.ispackaged()
                ):
                ...
                for table in self.tables:
                    script.append(
                        table.get_sql_for("create", no_pk=True, no_unique=True)
                    )
                ...
                for table in self.tables:
                    for index in (x for x in table.indices if not x.isenforcer()):
                ...
                for priv in (
                    ...
                    if x.user_name != "SYSDBA" and not x.subject.issystemobject()
                ):
    ...
    def get_collation_by_id(self, charset_id, collation_id):
        ...
        """
        for collation in self.collations:
            if (collation._attributes["RDB$CHARACTER_SET_ID"] == charset_id) and (
                collation.id == collation_id
            ):
                return collation
        else:
            return None
...
class BaseSchemaItem(Visitable):
    ...
    def __init__(self, schema, attributes):
        self.schema = (
            schema if isinstance(schema, weakref.ProxyType) else weakref.proxy(schema)
        )
        self._type_code = []
    ...
    def _check_params(self, params, param_names):
        ...
        if not p.issubset(n):
            raise fdb.ProgrammingError(
                "Unsupported parameter(s) '%s'" % ",".join(p.difference(n))
            )
    ...
    # --- properties
    name = LateBindingProperty(
        _get_name, doc="Database object name or None if object doesn't have a name."
    )
    description = LateBindingProperty(
        _get_description,
        doc="Database object description or None if object doesn't have a description.",
    )
    actions = LateBindingProperty(
        _get_actions,
        doc="List of supported SQL operations on metadata object instance.",
    )
    # --- Public
    ...
    def get_dependents(self):
        ...
        return [
            d
            for d in self.schema.dependencies
            if d.depended_on_name == self.name and d.depended_on_type in self._type_code
        ]
    ...
    def get_dependencies(self):
        ...
        return [
            d
            for d in self.schema.dependencies
            if d.dependent_name == self.name and d.dependent_type in self._type_code
        ]
...
class Collation(BaseSchemaItem):
   ...
   %s""" % (
    ...
    def _get_comment_sql(self, **params):
        return "COMMENT ON COLLATION %s IS %s" % (
            self.get_quoted_name(),
            "NULL"
    ...
    def _get_character_set(self):
        ...
        return self.schema.get_character_set_by_id(
            self._attributes["RDB$CHARACTER_SET_ID"]
        )
    ...
    character_set = LateBindingProperty(
        _get_character_set, doc="Character set object associated with collation."
    )
    base_collation = LateBindingProperty(
        _get_base_collation,
        doc="Base Collation object that's extended by this one or None.",
    )
    attributes = LateBindingProperty(_get_attributes, doc="Collation attributes.")
    specific_attributes = LateBindingProperty(
        _get_specific_attributes, doc="Collation specific attributes."
    )
    function_name = LateBindingProperty(_get_function_name, doc="Not currently used.")
    # FB 3.0
    security_class = LateBindingProperty(
        _get_security_class, doc="Security class name or None."
    )
...
class CharacterSet(BaseSchemaItem):
    ...
    def _get_alter_sql(self, **params):
        ...
        if collation:
            return "ALTER CHARACTER SET %s SET DEFAULT COLLATION %s" % (
                self.get_quoted_name(),
    ...
    def _get_comment_sql(self, **params):
        return "COMMENT ON CHARACTER SET %s IS %s" % (
            self.get_quoted_name(),
            "NULL"
            if self.description is None
            else "'%s'" % escape_single_quotes(self.description),
    ...
    def _get_collations(self):
        return self.schema.collations.filter(
            lambda item: item._attributes["RDB$CHARACTER_SET_ID"] == self.id
        )
    ...
    bytes_per_character = LateBindingProperty(
        _get_bytes_per_character, doc="Size of characters in bytes."
    )
    default_collate = LateBindingProperty(
        _get_default_collate, doc="Collate object of default collate."
    )
    collations = LateBindingProperty(
        _get_collations,
        doc=":class:`~fdb.utils.ObjectList` of Collations associated with character set.",
    )
    # FB 3.0
    security_class = LateBindingProperty(
        _get_security_class, doc="Security class name or None."
...
class DatabaseException(BaseSchemaItem):
    ...
    def _get_comment_sql(self, **params):
        return "COMMENT ON EXCEPTION %s IS %s" % (
            self.get_quoted_name(),
            "NULL"
            if self.description is None
            else "'%s'" % escape_single_quotes(self.description),
...
class Sequence(BaseSchemaItem):
    ...
    def _get_create_sql(self, **params):
        self._check_params(params, [])
        return "CREATE %s %s" % (
            self.schema.opt_generator_keyword,
            self.get_quoted_name(),
        )
    ...
    def _get_alter_sql(self, **params):
        ...
        if value is not None:
            return "ALTER %s %s RESTART WITH %d" % (
                self.schema.opt_generator_keyword,
                self.get_quoted_name(),
                value,
            )
    ...
    def _get_value(self):
        return self.schema._select_row(
            "select GEN_ID(%s,0) from RDB$DATABASE" % self.get_quoted_name()
        )["GEN_ID"]
...
class TableColumn(BaseSchemaItem):
    ...
    def _get_alter_sql(self, **params):
        self._check_params(
            params, ["expression", "datatype", "name", "position", "restart"]
        )
        ...
        sql = "ALTER TABLE %s ALTER COLUMN %s" % (
            self.table.get_quoted_name(),
            self.get_quoted_name(),
        )
        if new_name:
    ...
    def _get_collation(self):
        return self.schema.get_collation_by_id(
            self.domain._attributes["RDB$CHARACTER_SET_ID"],
    ...
    def _get_privileges(self):
        return self.schema.privileges.filter(
            lambda p: (
                p.subject_name == self.table.name
                and p.field_name == self.name
                and p.subject_type in self.table._type_code
            )
        )
    ...
    table = LateBindingProperty(
        _get_table, doc="The Table object this column belongs to."
    )
    domain = LateBindingProperty(
        _get_domain, doc="Domain object this column is based on."
    )
    position = LateBindingProperty(
        _get_position, doc="Column's sequence number in row."
    )
    security_class = LateBindingProperty(
        _get_security_class, doc="Security class name or None."
    )
    ...
    datatype = LateBindingProperty(
        _get_datatype, doc="Comlete SQL datatype definition."
    )
    privileges = LateBindingProperty(
        _get_privileges,
        doc=":class:`~fdb.utils.ObjectList` of :class:`Privilege` objects granted to this object.",
    )
    # FB 3.0
    ...
    def get_dependents(self):
        ...
        return [
            ...
            and d.depended_on_type == 0
            and d.field_name == self.name
        ]
...
class Index(BaseSchemaItem):
    ...
    def _get_comment_sql(self, **params):
        return "COMMENT ON INDEX %s IS %s" % (
            self.get_quoted_name(),
            "NULL"
            if self.description is None
            else "'%s'" % escape_single_quotes(self.description),
    ...
    def _get_index_type(self):
        return (
            ...
            if self._attributes["RDB$INDEX_TYPE"] == 1
    ...
    def _get_segments(self):
        return ObjectList(
            self.table.get_column(colname) for colname in self.segment_names
        )
        ...
        if self.__segment_names is None:
            if self._attributes["RDB$SEGMENT_COUNT"] > 0:
                self.__segment_names = [
                    r["RDB$FIELD_NAME"].strip()
                    for r in self.schema._select(
                        """select rdb$field_name
from rdb$index_segments where rdb$index_name = ? order by rdb$field_position""",
                        (self.name,),
                    )
                ]
            else:
...
from rdb$index_segments where rdb$index_name = ? order by rdb$field_position""",
                    ...
                    self.__segment_statistics = [
                        None for x in range(self._attributes["RDB$SEGMENT_COUNT"])
                    ]
            else:
                self.__segment_statistics = []
    ...
    # --- Properties
    table = LateBindingProperty(
        _get_table, doc="The :class:`Table` instance the index applies to."
    )
    ...
    partner_index = LateBindingProperty(
        _get_partner_index,
        doc="Associated unique/primary key :class:`Index` instance, or None.",
    )
    expression = LateBindingProperty(
        _get_expression, doc="Source of an expression or None."
    )
    statistics = LateBindingProperty(
        _get_statistics, doc="Latest selectivity of the index."
    )
    segment_names = LateBindingProperty(
        _get_segment_names, doc="List of index segment names."
    )
    segment_statistics = LateBindingProperty(
        _get_segment_statistics,
        doc="List of index segment statistics (for ODS 11.1 and higher).",
    )
    segments = LateBindingProperty(
        _get_segments,
        doc=":class:`~fdb.utils.ObjectList` of index segments as :class:`TableColumn` instances.",
    )
    constraint = LateBindingProperty(
        _get_constraint,
        doc=":class:`Constraint` instance that uses this index or None.",
    )
    # --- Public
    def issystemobject(self):
        "Returns True if this database object is system object."
        return bool(
            self._attributes["RDB$SYSTEM_FLAG"]
...
class ViewColumn(BaseSchemaItem):
    ...
    def _get_privileges(self):
        return self.schema.privileges.filter(
            lambda p: (
                p.subject_name == self.view.name
                and p.field_name == self.name
                and p.subject_type == 0
            )
        )  # Views are logged as Tables in RDB$USER_PRIVILEGES

    # --- Properties
    base_field = LateBindingProperty(
        _get_base_field,
        doc="The source column from the base relation. Result could be either "
...
class Domain(BaseSchemaItem):
    ...
    def _get_create_sql(self, **params):
        ...
        if self._attributes["RDB$COLLATION_ID"]:
            ...
            if (
                self.character_set._attributes["RDB$DEFAULT_COLLATE_NAME"]
                != self.collation.name
            ):
                sql += " COLLATE %s" % self.collation.get_quoted_name()
        return sql
    ...
    def _get_alter_sql(self, **params):
        ...
        if new_name:
            ...
            return (
                "%s SET DEFAULT %s" % (sql, new_default)
                if new_default
                else "%s DROP DEFAULT" % sql
            )
        elif new_constraint != "":
            return (
                "%s ADD CHECK (%s)" % (sql, new_constraint)
                if new_constraint
    ...
    def _get_comment_sql(self, **params):
        return "COMMENT ON DOMAIN %s IS %s" % (
            self.get_quoted_name(),
            "NULL"
            if self.description is None
            else "'%s'" % escape_single_quotes(self.description),
    ...
    def _get_datatype(self):
            ...
            if self.precision != None:
                if (self.sub_type > 0) and (self.sub_type <= MAX_INTSUBTYPES):
                    l.append(
                        "%s(%d, %d)"
                        % (
                            INTEGRAL_SUBTYPES[self.sub_type],
                            self.precision,
                            -self.scale,
                        )
                    )
        ...
        if self.field_type in (FBT_CHAR, FBT_VARCHAR):
            l.append(
                ...
                % (
                    self.length
                    if self.character_length is None
        ...
        if self._attributes["RDB$DIMENSIONS"] != None:
            l.append(
                "[%s]"
                % ", ".join(
                    "%d" % u if l == 1 else "%d:%d" % (l, u) for l, u in self.dimensions
                )
            )
        ...
        if self.field_type in (FBT_CHAR, FBT_VARCHAR, FBT_BLOB):
            if (
                self._attributes["RDB$CHARACTER_SET_ID"] is not None
    ...
    # --- Properties
    expression = LateBindingProperty(
        _get_expression, doc="Expression that defines the COMPUTED BY column or None."
    )
    validation = LateBindingProperty(
        _get_validation, doc="CHECK constraint for the domain or None."
    )
    default = LateBindingProperty(
        _get_default, doc="Expression that defines the default value or None."
    )
    length = LateBindingProperty(_get_length, doc="Length of the column in bytes.")
    scale = LateBindingProperty(
        _get_scale,
        doc="Negative number representing the scale of NUMBER and DECIMAL column.",
    )
    field_type = LateBindingProperty(
        _get_field_type, doc="Number code of the data type defined for the column."
    )
    sub_type = LateBindingProperty(_get_sub_type, doc="BLOB subtype.")
    segment_length = LateBindingProperty(
        _get_segment_length,
        doc="For BLOB columns, a suggested length for BLOB buffers.",
    )
    external_length = LateBindingProperty(
        _get_external_length,
        doc="Length of field as it is in an external table. Always 0 for regular tables.",
    )
    external_scale = LateBindingProperty(
        _get_external_scale,
        doc="Scale factor of an integer field as it is in an external table.",
    )
    external_type = LateBindingProperty(
        _get_external_type, doc="Data type of the field as it is in an external table."
    )
    dimensions = LateBindingProperty(
        _get_dimensions,
        doc="List of dimension definition pairs if column is an array type. Always empty for non-array columns.",
    )
    character_length = LateBindingProperty(
        _get_character_length,
        doc="Length of CHAR and VARCHAR column, in characters (not bytes).",
    )
    collation = LateBindingProperty(
        _get_collation, doc="Collation object for a character column or None."
    )
    character_set = LateBindingProperty(
        _get_character_set,
        doc="CharacterSet object for a character or text BLOB column, or None.",
    )
    precision = LateBindingProperty(
        _get_precision,
        doc="Indicates the number of digits of precision available to the data type of the column.",
    )
    datatype = LateBindingProperty(
        _get_datatype, doc="Comlete SQL datatype definition."
    )
    ...
    def issystemobject(self):
        ...
        return (self._attributes["RDB$SYSTEM_FLAG"] == 1) or self.name.startswith(
            "RDB$"
        )
...
class Dependency(BaseSchemaItem):
    ...
    dependent_name = LateBindingProperty(
        _get_dependent_name, doc="Dependent database object name."
    )
    dependent_type = LateBindingProperty(
        _get_dependent_type, doc="Dependent database object type."
    )
    field_name = LateBindingProperty(
        _get_field_name, doc="Name of one column in `depended on` object."
    )
    depended_on = LateBindingProperty(
        _get_depended_on, doc="Database object on which dependent depends."
    )
    depended_on_name = LateBindingProperty(
        _get_depended_on_name, doc="Name of db object on which dependent depends."
    )
    depended_on_type = LateBindingProperty(
        _get_depended_on_type, doc="Type of db object on which dependent depends."
    )
    # FB 3.0
    package = LateBindingProperty(
        _get_package,
        doc=":class:`Package` instance if dependent depends on object in package or None.",
    )
    # --- Public
    def issystemobject(self):
...
class Constraint(BaseSchemaItem):
    ...
    def _get_create_sql(self, **params):
        ...
        if self.ischeck():
            ...
            const_def += "REFERENCES %s (%s)" % (
                p.table.get_quoted_name(),
            ...
            raise fdb.OperationalError(
                "Unrecognized constraint type '%s'" % self.constraint_type
            )
        return const_def
    ...
    # --- Properties
    constraint_type = LateBindingProperty(
        _get_constraint_type, doc="primary key/unique/foreign key/check/not null."
    )
    table = LateBindingProperty(
        _get_table, doc=":class:`Table` instance this constraint applies to."
    )
    index = LateBindingProperty(
        _get_index,
        doc=":class:`Index` instance that enforces the constraint.\n`None` if constraint is not primary key/unique or foreign key.",
    )
    trigger_names = LateBindingProperty(
        _get_trigger_names,
        doc="For a CHECK constraint contains trigger names that enforce the constraint.",
    )
    triggers = LateBindingProperty(
        _get_triggers,
        doc="For a CHECK constraint contains :class:`Trigger` instances that enforce the constraint.",
    )
    column_name = LateBindingProperty(
        _get_column_name,
        doc="For a NOT NULL constraint, this is the name of the column to which the constraint applies.",
    )
    partner_constraint = LateBindingProperty(
        _get_partner_constraint,
        doc="For a FOREIGN KEY constraint, this is the unique or primary key :class:`Constraint` referred.",
    )
    match_option = LateBindingProperty(
        _get_match_option,
        doc="For a FOREIGN KEY constraint only. Current value is FULL in all cases.",
    )
    update_rule = LateBindingProperty(
        _get_update_rule,
        doc="For a FOREIGN KEY constraint, this is the action applicable to when primary key is updated.",
    )
    delete_rule = LateBindingProperty(
        _get_delete_rule,
        doc="For a FOREIGN KEY constraint, this is the action applicable to when primary key is deleted.",
    )
    # --- Public
    def issystemobject(self):
        "Returns True if this database object is system object."
        return self.schema.get_table(
            self._attributes["RDB$RELATION_NAME"]
        ).issystemobject()
...
class Table(BaseSchemaItem):
    ...
    def _get_create_sql(self, **params):
        try:
            ...
            #
            tabdef = "CREATE %sTABLE %s" % (
                "GLOBAL TEMPORARY " if self.isgtt() else "",
                self.get_quoted_name(),
            )
            if self.isexternal():
                ...
                if col.isidentity():
                    ...
                    if col._attributes["RDB$COLLATION_ID"] is not None:
                        ...
                        if col.collation is not None:
                            ...
                            if (
                                col.domain.character_set._attributes[
    ...
    def _get_table_type(self):
        return self.schema.enum_relation_types.get(
            self._attributes.get("RDB$RELATION_TYPE"), "PERSISTENT"
        )
    ...
    def _get_indices(self):
        ...
        result = user_indices.filter(
            lambda i: i._attributes["RDB$RELATION_NAME"] == self.name
        )
        result.extend(
            sys_indices.filter(
                lambda i: i._attributes["RDB$RELATION_NAME"] == self.name
    ...
    def _get_triggers(self):
        return self.schema.triggers.filter(
            lambda t: t._attributes["RDB$RELATION_NAME"] == self.name
        )

    def _get_constraints(self):
        return self.schema.constraints.filter(
            lambda c: c._attributes["RDB$RELATION_NAME"] == self.name
        )
    ...
    def _get_columns(self):
        if self.__columns is None:
            ...
            self.__columns = ObjectList(
                (
                    TableColumn(self.schema, self, row)
                    for row in self.schema._select(
                        """select %s from RDB$RELATION_FIELDS
where RDB$RELATION_NAME = ? order by RDB$FIELD_POSITION"""
    ...
    def _get_privileges(self):
        return self.schema.privileges.filter(
            lambda p: (
                (p.subject_name == self.name) and (p.subject_type in self._type_code)
            )
        )

    # --- Properties
    id = LateBindingProperty(_get_id, doc="Internam number ID for the table.")
    dbkey_length = LateBindingProperty(
        _get_dbkey_length, doc="Length of the RDB$DB_KEY column in bytes."
    )
    ...
    security_class = LateBindingProperty(
        _get_security_class,
        doc="Security class that define access limits to the table.",
    )
    external_file = LateBindingProperty(
        _get_external_file, doc="Full path to the external data file, if any."
    )
    owner_name = LateBindingProperty(
        _get_owner_name, doc="User name of table's creator."
    )
    default_class = LateBindingProperty(
        _get_default_class, doc="Default security class."
    )
    flags = LateBindingProperty(_get_flags, doc="Internal flags.")
    primary_key = LateBindingProperty(
        _get_primary_key, doc="PRIMARY KEY :class:`Constraint` for this table or None."
    )
    foreign_keys = LateBindingProperty(
        _get_foreign_keys,
        doc=":class:`~fdb.utils.ObjectList` of FOREIGN KEY :class:`Constraint` instances for this table.",
    )
    columns = LateBindingProperty(
        _get_columns,
        doc="Returns :class:`~fdb.utils.ObjectList` of columns defined for table.\nItems are :class:`TableColumn` objects.",
    )
    constraints = LateBindingProperty(
        _get_constraints,
        doc="Returns :class:`~fdb.utils.ObjectList` of constraints defined for table.\nItems are :class:`Constraint` objects.",
    )
    indices = LateBindingProperty(
        _get_indices,
        doc="Returns :class:`~fdb.utils.ObjectList` of indices defined for table.\nItems are :class:`Index` objects.",
    )
    triggers = LateBindingProperty(
        _get_triggers,
        doc="Returns :class:`~fdb.utils.ObjectList` of triggers defined for table.\nItems are :class:`Trigger` objects.",
    )
    privileges = LateBindingProperty(
        _get_privileges,
        doc=":class:`~fdb.utils.ObjectList` of :class:`Privilege` objects granted to this object.",
...
class View(BaseSchemaItem):
    ...
    def _get_columns(self):
        if self.__columns is None:
            self.__columns = ObjectList(
                (
                    ViewColumn(self.schema, self, row)
                    for row in self.schema._select(
...
v.RDB$RELATION_NAME as BASE_RELATION
    ...
    def _get_privileges(self):
        return self.schema.privileges.filter(
            lambda p: ((p.subject_name == self.name) and (p.subject_type == 0))
        )  # Views are logged as Tables in RDB$USER_PRIVILEGES
    ...
    security_class = LateBindingProperty(
        _get_security_class, doc="Security class that define access limits to the view."
    )
    owner_name = LateBindingProperty(
        _get_owner_name, doc="User name of view's creator."
    )
    default_class = LateBindingProperty(
        _get_default_class, doc="Default security class."
    )
    ...
    columns = LateBindingProperty(
        _get_columns,
        doc="Returns :class:`~fdb.utils.ObjectList` of columns defined for view.\nItems are :class:`ViewColumn` objects.",
    )
    triggers = LateBindingProperty(
        _get_triggers,
        doc="Returns :class:`~fdb.utils.ObjectList` of triggers defined for view.\nItems are :class:`Trigger` objects.",
    )
    privileges = LateBindingProperty(
        _get_privileges,
        doc=":class:`~fdb.utils.ObjectList` of :class:`Privilege` objects granted to this object.",
...
class Trigger(BaseSchemaItem):
    ...
    def _get_create_sql(self, **params):
        ...
        result += " %s\n%s POSITION %d\n%s" % (
            "ACTIVE" if self.isactive() and not inactive else "INACTIVE",
    ...
    def _get_alter_sql(self, **params):
        ...
        if action is not None:
            ...
            if (dbaction and not self.isdbtrigger()) or (
                not dbaction and self.isdbtrigger()
            ):
    ...
    def _get_comment_sql(self, **params):
        return "COMMENT ON TRIGGER %s IS %s" % (
            self.get_quoted_name(),
            "NULL"
            if self.description is None
            else "'%s'" % escape_single_quotes(self.description),
    ...
    relation = LateBindingProperty(
        _get_relation,
        doc=":class:`Table` or :class:`View` that the trigger is for, or None for database triggers",
    )
    sequence = LateBindingProperty(
        _get_sequence,
        doc="Sequence (position) of trigger. Zero usually means no sequence defined.",
    )
    trigger_type = LateBindingProperty(
        _get_trigger_type,
        doc="Numeric code for trigger type that define what event and when are covered by trigger.",
    )
    source = LateBindingProperty(_get_source, doc="PSQL source code.")
    flags = LateBindingProperty(_get_flags, doc="Internal flags.")
    valid_blr = LateBindingProperty(
        _get_valid_blr, doc="Trigger BLR invalidation flag. Coul be True/False or None."
    )
    # FB 3
    ...
    def get_type_as_string(self):
        ...
        if self.isddltrigger():
            ...
            l.append(
                "ANY DDL STATEMENT"
                if code == DDL_TRIGGER_ANY
                else TRIGGER_DDL_TYPES[code]
            )
...
class ProcedureParameter(BaseSchemaItem):
    ...
    # --- Protected
    def _get_comment_sql(self, **params):
        return "COMMENT ON PARAMETER %s.%s IS %s" % (
            self.procedure.get_quoted_name(),
            self.get_quoted_name(),
            "NULL"
            if self.description is None
            else "'%s'" % escape_single_quotes(self.description),
    ...
    def _get_collation(self):
        ...
        return (
            None
            if cid is None
            else self.schema.get_collation_by_id(
                self.domain._attributes["RDB$CHARACTER_SET_ID"], cid
            )
        )
    ...
    def _get_column(self):
        ...
        return (
            None
            if rname is None
            else self.schema.get_table(rname).get_column(
                self._attributes["RDB$FIELD_NAME"]
            )
        )
    ...
    sequence = LateBindingProperty(
        _get_sequence, doc="Sequence (position) of parameter."
    )
    domain = LateBindingProperty(_get_domain, doc=":class:`Domain` for this parameter.")
    datatype = LateBindingProperty(
        _get_datatype, doc="Comlete SQL datatype definition."
    )
    type_from = LateBindingProperty(
        _get_type_from, doc="Numeric code. See :attr:`Schema.enum_param_type_from`.`"
    )
    # FB 2.1
    default = LateBindingProperty(_get_default, doc="Default value.")
    collation = LateBindingProperty(
        _get_collation, doc=":class:`collation` for this parameter."
    )
    mechanism = LateBindingProperty(_get_mechanism, doc="Parameter mechanism code.")
    # FB 2.5
    column = LateBindingProperty(
        _get_column, doc=":class:`TableColumn` for this parameter."
    )
    # FB 3.0
    package = LateBindingProperty(
        _get_package,
        doc="Package this procedure belongs to. \nObject is :class:`Package` instance or None.",
    )
    # --- Public
    def get_sql_definition(self):
        ...
        if self.type_from == PROCPAR_DOMAIN:
            ...
            typedef = "TYPE OF COLUMN %s.%s" % (
                self.column.table.get_quoted_name(),
...
class Procedure(BaseSchemaItem):
    ...
    def _get_comment_sql(self, **params):
        return "COMMENT ON PROCEDURE %s IS %s" % (
            self.get_quoted_name(),
            "NULL"
            if self.description is None
            else "'%s'" % escape_single_quotes(self.description),
        ...
        if self.__input_params is None:
            if self.has_input():
                self.__input_params = ObjectList(
                    (
                        ProcedureParameter(self.schema, self, row)
                        for row in self.schema._select(
                            sql % self.__param_columns(), (self.name,)
                        )
                    ),
                    ProcedureParameter,
                    "item.name",
...
and rdb$parameter_type = 1 order by rdb$parameter_number"""
    ...
    security_class = LateBindingProperty(
        _get_security_class,
        doc="Security class that define access limits to the procedure.",
    )
    owner_name = LateBindingProperty(
        _get_owner_name, doc="User name of procedure's creator."
    )
    input_params = LateBindingProperty(
        _get_input_params,
        doc=":class:`~fdb.utils.ObjectList` of input parameters.\nInstances are :class:`ProcedureParameter` instances.",
    )
    output_params = LateBindingProperty(
        _get_output_params,
        doc=":class:`~fdb.utils.ObjectList` of output parameters.\nInstances are :class:`ProcedureParameter` instances.",
    )
    privileges = LateBindingProperty(
        _get_privileges,
        doc=":class:`~fdb.utils.ObjectList` of :class:`Privilege` objects granted to this object.",
    ...
    # FB 2.1
    proc_type = LateBindingProperty(
        _get_proc_type,
        doc="Procedure type code. See :attr:`~fdb.schema.Schema.enum_procedure_types`.",
    )
    valid_blr = LateBindingProperty(
        _get_valid_blr,
        doc="Procedure BLR invalidation flag. Coul be True/False or None.",
    )
    # FB 3.0
    engine_name = LateBindingProperty(_get_engine_name, doc="Engine name.")
    entrypoint = LateBindingProperty(_get_entrypoint, doc="Entrypoint.")
...
class Role(BaseSchemaItem):
    ...
    def _get_privileges(self):
        return self.schema.privileges.filter(
            lambda p: ((p.user_name == self.name) and (p.user_type in self._type_code))
        )
...
class FunctionArgument(BaseSchemaItem):
    ...
    def _get_name(self):
        return (
            ...
            else (self.function.name + "_" + str(self._get_position()))
        )
    ...
    field_type = LateBindingProperty(
        _get_field_type, doc="Number code of the data type defined for the argument."
    )
    length = LateBindingProperty(_get_length, doc="Length of the argument in bytes.")
    scale = LateBindingProperty(
        _get_scale,
        doc="Negative number representing the scale of NUMBER and DECIMAL argument.",
    )
    precision = LateBindingProperty(
        _get_precision,
        doc="Indicates the number of digits of precision available to the data type of the argument.",
    )
    sub_type = LateBindingProperty(_get_sub_type, doc="BLOB subtype.")
    character_length = LateBindingProperty(
        _get_character_length,
    ...
    character_set = LateBindingProperty(
        _get_character_set,
        doc=":class:`CharacterSet` for a character/text BLOB argument, or None.",
    )
    datatype = LateBindingProperty(
        _get_datatype, doc="Comlete SQL datatype definition."
    )
    ...
    argument_mechanism = LateBindingProperty(
        _get_argument_mechanism, doc="Argument mechanism."
    )
    column = LateBindingProperty(
        _get_column, doc=":class:`TableColumn` for this parameter."
    )
    ...
    def get_sql_definition(self):
        ...
        if self.function.isexternal():
            ...
            result = "%s%s%s" % (
                self.get_quoted_name() + " " if not self.isreturning() else "",
                typedef,
                "" if self.isnullable() else " NOT NULL",
            )
            c = self.collation
...
class Function(BaseSchemaItem):
    ...
    def _get_drop_sql(self, **params):
        self._check_params(params, [])
        return "DROP%s FUNCTION %s" % (
            " EXTERNAL" if self.isexternal() else "",
            self.get_quoted_name(),
        )

    def _get_comment_sql(self, **params):
        return "COMMENT ON EXTERNAL FUNCTION %s IS %s" % (
            self.get_quoted_name(),
            "NULL"
            if self.description is None
            else "'%s'" % escape_single_quotes(self.description),
    ...
    def _load_arguments(self, mock=None):
        ...
        self.__arguments = ObjectList(
            (
                FunctionArgument(self.schema, self, row)
                for row in (
                    mock
                    if mock
                    else self.schema._select(
                        """select %s from rdb$function_arguments
where rdb$function_name = ? order by rdb$argument_position"""
    ...
    returns = LateBindingProperty(
        _get_returns, doc="Returning :class:`FunctionArgument` or None."
    )
    arguments = LateBindingProperty(
        _get_arguments,
    ...
    deterministic_flag = LateBindingProperty(
        _get_deterministic_flag, doc="Deterministic flag."
    )
    # --- Public
    def isexternal(self):
...
class Shadow(BaseSchemaItem):
    ...
    def _get_create_sql(self, **params):
        self._check_params(params, [])
        result = "CREATE SHADOW %d %s%s" % (
            self.id,
            "MANUAL" if self.ismanual() else "AUTO",
            " CONDITIONAL" if self.isconditional() else "",
        )
        if len(self.files) == 1:
...
order by RDB$FILE_SEQUENCE""",
                    (self._attributes["RDB$SHADOW_NUMBER"],),
                )
            ]
        return self.__files
...
class Privilege(BaseSchemaItem):
    ...
    def _get_revoke_sql(self, **params):
        ...
        if self.privilege in privileges:
            ...
            privilege = ""
            admin_option = (
                "ADMIN OPTION FOR" if self.has_grant() and option_only else ""
            )
        user = self.user
        if isinstance(user, Procedure):
            utype = "PROCEDURE "
        ...
        if (grantors is not None) and (self.grantor_name not in grantors):
            ...
            granted_by = ""
        return "REVOKE %s%s%s FROM %s%s%s" % (
            admin_option,
            privilege,
            self.subject_name,
            utype,
    ...
    def _get_user(self):
        return self.schema._get_item(
            self._attributes["RDB$USER"], self._attributes["RDB$USER_TYPE"]
        )
...
class Package(BaseSchemaItem):
    ...
    def _get_functions(self):
        return self.schema.functions.filter(
            lambda fn: fn._attributes["RDB$PACKAGE_NAME"] == self.name
        )

    def _get_procedures(self):
        return self.schema.procedures.filter(
            lambda proc: proc._attributes["RDB$PACKAGE_NAME"] == self.name
        )
    ...
    owner_name = LateBindingProperty(
        _get_owner_name, doc="User name of package creator."
    )
    functions = LateBindingProperty(
        _get_functions,
...
class BackupHistory(BaseSchemaItem):
    ...
    # --- Properties
    backup_id = LateBindingProperty(
        _get_backup_id, doc="The identifier assigned by the engine."
    )
    filename = LateBindingProperty(
        _get_filename, doc="Full path and file name of backup file."
    )
...
class Filter(BaseSchemaItem):
    ...
    def _get_declare_sql(self, **params):
        ...
        return "%sENTRY_POINT '%s' MODULE_NAME '%s'" % (
            fdef,
            self.entrypoint,
            self.module_name,
        )
    ...
    def _get_comment_sql(self, **params):
        return "COMMENT ON FILTER %s IS %s" % (
            self.get_quoted_name(),
            "NULL"
            if self.description is None
            else "'%s'" % escape_single_quotes(self.description),
    ...
    # --- Properties
    module_name = LateBindingProperty(
        _get_module_name,
        doc="The name of the dynamic library or shared object where the code of the BLOB filter is located.",
    )
    entrypoint = LateBindingProperty(
        _get_entrypoint,
        doc="The exported name of the BLOB filter in the filter library.",
    )
    input_sub_type = LateBindingProperty(
        _get_input_sub_type,
        doc="The BLOB subtype of the data to be converted by the function.",
    )
    output_sub_type = LateBindingProperty(
        _get_output_sub_type, doc="The BLOB subtype of the converted data."
    )
