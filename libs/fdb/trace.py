...
AttachmentInfo = collections.namedtuple(
    "AttachmentInfo",
    "attachment_id,database,charset,protocol,address,user,role,remote_process,remote_pid",
)
TransactionInfo = collections.namedtuple(
    "TransactionInfo", "attachment_id,transaction_id,options"
)
ServiceInfo = collections.namedtuple(
    "ServiceInfo", "service_id,user,protocol,address,remote_process,remote_pid"
)
SQLInfo = collections.namedtuple("SQLInfo", "sql_id,sql,plan")
ParamInfo = collections.namedtuple("ParamInfo", "par_id,params")
#
AccessTuple = collections.namedtuple(
    "AccessTuple", "table,natural,index,update,insert,delete,backout,purge,expunge"
)
#
EventTraceInit = collections.namedtuple(
    "EventTraceInit", "event_id,timestamp,session_name"
)
EventTraceSuspend = collections.namedtuple(
    "EventTraceSuspend", "event_id,timestamp,session_name"
)
EventTraceFinish = collections.namedtuple(
    "EventTraceFinish", "event_id,timestamp,session_name"
)
#
EventCreate = collections.namedtuple(
    "EventCreate",
    "event_id,timestamp,status,attachment_id,database,charset,protocol,address,user,role,remote_process,remote_pid",
)
EventDrop = collections.namedtuple(
    "EventDrop",
    "event_id,timestamp,status,attachment_id,database,charset,protocol,address,user,role,remote_process,remote_pid",
)
EventAttach = collections.namedtuple(
    "EventAttach",
    "event_id,timestamp,status,attachment_id,database,charset,protocol,address,user,role,remote_process,remote_pid",
)
EventDetach = collections.namedtuple(
    "EventDetach",
    "event_id,timestamp,status,attachment_id,database,charset,protocol,address,user,role,remote_process,remote_pid",
)
#
EventTransactionStart = collections.namedtuple(
    "EventTransactionStart",
    "event_id,timestamp,status,attachment_id,transaction_id,options",
)
EventCommit = collections.namedtuple(
    "EventCommit",
    "event_id,timestamp,status,attachment_id,transaction_id,options,run_time,reads,writes,fetches,marks",
)
EventRollback = collections.namedtuple(
    "EventRollback",
    "event_id,timestamp,status,attachment_id,transaction_id,options,run_time,reads,writes,fetches,marks",
)
EventCommitRetaining = collections.namedtuple(
    "EventCommitRetaining",
    "event_id,timestamp,status,attachment_id,transaction_id,options,run_time,reads,writes,fetches,marks",
)
EventRollbackRetaining = collections.namedtuple(
    "EventRollbackRetaining",
    "event_id,timestamp,status,attachment_id,transaction_id,options,run_time,reads,writes,fetches,marks",
)
#
EventPrepareStatement = collections.namedtuple(
    "EventPrepareStatement",
    "event_id,timestamp,status,attachment_id,transaction_id,statement_id,sql_id,prepare_time",
)
EventStatementStart = collections.namedtuple(
    "EventStatementStart",
    "event_id,timestamp,status,attachment_id,transaction_id,statement_id,sql_id,param_id",
)
EventStatementFinish = collections.namedtuple(
...
#
EventServiceAttach = collections.namedtuple(
    "EventServiceAttach", "event_id,timestamp,status,service_id"
)
EventServiceDetach = collections.namedtuple(
    "EventServiceDetach", "event_id,timestamp,status,service_id"
)
EventServiceStart = collections.namedtuple(
    "EventServiceStart", "event_id,timestamp,status,service_id,action,parameters"
)
EventServiceQuery = collections.namedtuple(
    "EventServiceQuery", "event_id,timestamp,status,service_id,action,parameters"
)
#
EventSetContext = collections.namedtuple(
    "EventSetContext",
    "event_id,timestamp,attachment_id,transaction_id,context,key,value",
)
#
EventError = collections.namedtuple(
    "EventError", "event_id,timestamp,attachment_id,place,details"
)
EventServiceError = collections.namedtuple(
    "EventServiceError", "event_id,timestamp,service_id,place,details"
)
EventWarning = collections.namedtuple(
    "EventWarning", "event_id,timestamp,attachment_id,place,details"
)
EventServiceWarning = collections.namedtuple(
    "EventServiceWarning", "event_id,timestamp,service_id,place,details"
)
#
EventSweepStart = collections.namedtuple(
    "EventSweepStart", "event_id,timestamp,attachment_id,oit,oat,ost,next"
)
EventSweepProgress = collections.namedtuple(
    "EventSweepProgress",
    "event_id,timestamp,attachment_id,run_time,reads,writes,fetches,marks,access",
)
EventSweepFinish = collections.namedtuple(
    "EventSweepFinish",
    "event_id,timestamp,attachment_id,oit,oat,ost,next,run_time,reads,writes,fetches,marks",
)

EventSweepFailed = collections.namedtuple(
    "EventSweepFailed", "event_id,timestamp,attachment_id"
)
#
EventBLRCompile = collections.namedtuple(
    "EventBLRCompile",
    "event_id,timestamp,status,attachment_id,statement_id,content,prepare_time",
)
EventBLRExecute = collections.namedtuple(
    "EventBLRExecute",
    "event_id,timestamp,status,attachment_id,transaction_id,statement_id,content,run_time,reads,writes,fetches,marks,access",
)
EventDYNExecute = collections.namedtuple(
    "EventDYNExecute",
...
class TraceParser(object):
    ...
    def __init__(self):
        ...
        self.__event_values = {}
        self.__parse_map = {
            EVENT_TRACE_INIT: self.__parser_trace_init,
    ...
    def _parse_header(self, line):
        ...
        if (len(items) == 3) or (items[2] in ["ERROR", "WARNING"]):
            return (
                timestamp,
                STATUS_OK,
                EVENTS.index(items[2]) if items[2] in EVENTS else EVENT_UNKNOWN,
            )
        else:
    ...
    def _parse_attachment_info(self, values, check=True):
        ...
        attachment_id, user_role, charset, protocol_address = attachment.strip(
            "()"
        ).split(",")
        ...
        elif len(self.__current_block) > 0 and not (
            self.__current_block[0].startswith("(TRA")
            or " ms," in self.__current_block[0]
            or "Transaction counters:" in self.__current_block[0]
        ):
    ...
    def _parse_transaction_info(self, values, check=True):
        ...
        transaction_id, transaction_options = (
            self.__current_block.popleft().strip("\t ()").split(",")
        )
            ...
            while values:
                ...
                if "ms" in val_type:
                    ...
                    raise fdb.ParseError(
                        "Unhandled performance parameter %s" % val_type
                    )
    ...
    def _parse_parameters(self, for_procedure=False):
        ...
        while self.__current_block and self.__current_block[0].endswith(
            "more arguments skipped..."
        ):
            self.__current_block.popleft()
        #
        param_id = None
        if len(parameters) > 0:
            ...
            if key in self.param_map:
                ...
                self.__buffer.append(
                    ParamInfo(**{"par_id": param_id, "params": parameters})
                )
        #
    ...
    def __parse_trace_header(self):
        self.__last_timestamp, status, self.__current_event = self._parse_header(
            self.__current_block.popleft()
        )
