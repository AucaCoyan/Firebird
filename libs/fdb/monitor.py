    ...
    def _get_iostats(self):
        if self.__iostats is None:
            ...
            if self._con.ods >= fdb.ODS_FB_30:
                self._ic.execute(
                    """SELECT r.MON$STAT_ID, r.MON$STAT_GROUP,
        ...
        if self.__variables is None:
            ...
            if self._con.ods >= fdb.ODS_FB_25:
                ...
                self.__variables = ObjectList(
                    (ContextVariableInfo(self, row) for row in self._ic.itermap()),
                    ContextVariableInfo,
                    "item.stat_id",
                )
            else:
...
FROM MON$TABLE_STATS ts join MON$RECORD_STATS r
  on ts.MON$RECORD_STAT_ID = r.MON$STAT_ID"""
    ...
    db = LateBindingProperty(
        _get_database, doc=":class:`DatabaseInfo` object for attached database."
    )
...
class BaseInfoItem(object):
    ...
    def __init__(self, monitor, attributes):
        self.monitor = (
            monitor
            if isinstance(monitor, weakref.ProxyType)
            else weakref.proxy(monitor)
        )
...
class DatabaseInfo(BaseInfoItem):
    ...
    def __get_ods(self):
        return float(
            "%d.%d"
            % (self._attributes["MON$ODS_MAJOR"], self._attributes["MON$ODS_MINOR"])
        )
    ...
    oit = property(
        __get_oit, doc="Transaction ID of the oldest [interesting] transaction."
    )
    oat = property(__get_oat, doc="Transaction ID of the oldest active transaction.")
    ost = property(
        __get_ost,
        doc="Transaction ID of the Oldest Snapshot, i.e., the number of the OAT when the last garbage collection was done.",
    )
    next_transaction = property(
        __get_next_transaction,
        doc="Transaction ID of the next transaction that will be started.",
    )
    cache_size = property(
        __get_cache_size, doc="Number of pages allocated in the page cache."
    )
    ...
    sweep_interval = property(
        __get_sweep_interval,
        doc="The sweep interval configured in the database header. "
        "Value 0 indicates that sweeping is disabled.",
    )
    read_only = property(__get_read_only, doc="True if database is Read Only.")
    forced_writes = property(
        __get_forced_writes, doc="True if database uses synchronous writes."
    )
    reserve_space = property(
        __get_reserve_space, doc="True if database reserves space on data pages."
    )
    created = property(
        __get_created,
        doc="Creation date and time, i.e., when the database was created or last restored.",
    )
    pages = property(__get_pages, doc="Number of pages allocated on disk.")
    backup_state = property(
        __get_backup_state,
        doc="Current state of database with respect to nbackup physical backup.",
    )
    ...
    security_database = property(
        __get_security_database,
        doc="Type of security database (Default, Self or Other).",
    )
    tablestats = property(
        __get_tablestats,
        doc="Dictionary of :class:`TableStatsInfo` instances for this object.",
    )
...
class AttachmentInfo(BaseInfoItem):
    ...
    def _get_transactions(self):
        return self.monitor.transactions.filter(
            lambda s: s._attributes["MON$ATTACHMENT_ID"] == self.id
        )

    def _get_statements(self):
        return self.monitor.statements.filter(
            lambda s: s._attributes["MON$ATTACHMENT_ID"] == self.id
        )

    def _get_variables(self):
        return self.monitor.variables.filter(
            lambda s: s._attributes["MON$ATTACHMENT_ID"] == self.id
        )
    ...
    remote_process = property(
        __get_remote_process, doc="Remote client process pathname."
    )
    character_set = property(
        __get_character_set,
        doc=":class:`~fdb.schema.CharacterSet` for this attachment.",
    )
    timestamp = property(__get_timestamp, doc="Attachment date/time.")
    transactions = LateBindingProperty(
        _get_transactions,
        doc=":class:`~fdb.utils.ObjectList` of transactions associated with attachment.\nItems are :class:`TransactionInfo` objects.",
    )
    statements = LateBindingProperty(
        _get_statements,
        doc=":class:`~fdb.utils.ObjectList` of statements associated with attachment.\nItems are :class:`StatementInfo` objects.",
    )
    variables = LateBindingProperty(
        _get_variables,
        doc=":class:`~fdb.utils.ObjectList` of variables associated with attachment.\nItems are :class:`ContextVariableInfo` objects.",
    )
    iostats = property(__get_iostats, doc=":class:`IOStatsInfo` for this object.")
    # FB 3.0
    ...
    remote_os_user = property(
        __get_remote_os_user, doc="OS user name of client process."
    )
    ...
    def terminate(self):
        ...
        if self.monitor._con.ods < fdb.ODS_FB_25:
            raise fdb.ProgrammingError(
                "Attachments could be terminated only "
            ...
            self.monitor._ic.execute(
                "delete from mon$attachments where mon$attachment_id = ?", (self.id,)
            )
...
class TransactionInfo(BaseInfoItem):
    ...
    def _get_statements(self):
        return self.monitor.statements.filter(
            lambda s: s._attributes["MON$TRANSACTION_ID"] == self.id
        )
    ...
    attachment = property(
        __get_attachment,
        doc=":class:`AttachmentInfo` instance to which this transaction belongs.",
    )
    ...
    oldest_active = property(
        __get_oldest_active, doc="Oldest active transaction (local OAT)."
    )
    isolation_mode = property(
        __get_isolation_mode, doc="Transaction isolation mode code."
    )
    lock_timeout = property(__get_lock_timeout, doc="Lock timeout.")
    statements = LateBindingProperty(
        _get_statements,
        doc=":class:`~fdb.utils.ObjectList` of statements associated with transaction.\nItems are :class:`StatementInfo` objects.",
    )
    variables = LateBindingProperty(
        _get_variables,
        doc=":class:`~fdb.utils.ObjectList` of variables associated with transaction.\nItems are :class:`ContextVariableInfo` objects.",
    )
    iostats = property(__get_iostats, doc=":class:`IOStatsInfo` for this object.")
    # FB 3.0
    tablestats = property(
...
class StatementInfo(BaseInfoItem):
    ...
    def __get_callstack(self):
        callstack = self.monitor.callstack.filter(
            lambda x: (
                (x._attributes["MON$STATEMENT_ID"] == self.id)
                and (x._attributes["MON$CALLER_ID"] is None)
            )
        )
        if len(callstack) > 0:
            item = callstack[0]
    ...
    attachment = property(
        __get_attachment,
        doc=":class:`AttachmentInfo` instance to which this statement belongs.",
    )
    transaction = property(
        __get_transaction,
        doc=":class:`TransactionInfo` instance to which this statement belongs or None.",
    )
    ...
    callstack = property(
        __get_callstack,
        doc=":class:`~fdb.utils.ObjectList` with call stack for statement.\nItems are :class:`CallStackInfo` objects.",
    )
    iostats = property(__get_iostats, doc=":class:`IOStatsInfo` for this object.")
    # FB 3.0
    ...
    def terminate(self):
        ...
        if self.attachment == self.monitor.this_attachment:
            raise fdb.ProgrammingError(
                "Can't terminate statement from current session."
            )
        else:
            self.monitor._ic.execute(
                "delete from mon$statements where mon$statement_id = ?", (self.id,)
            )
...
class CallStackInfo(BaseInfoItem):
    ...
    statement = property(
        __get_statement,
        doc="Top-level :class:`StatementInfo` instance to which this call stack entry belongs.",
    )
    caller = property(
        __get_caller, doc="Call stack entry (:class:`CallStackInfo`) of the caller."
    )
    dbobject = property(
        __get_dbobject,
        doc="PSQL object. :class:`~fdb.schema.Procedure` or :class:`~fdb.schema.Trigger` instance.",
    )
...
class IOStatsInfo(BaseInfoItem):
    ...
    owner = property(
        __get_owner,
        doc="""Object that owns this IOStats instance. Could be either
    ...
    backouts = property(
        __get_backouts,
        doc="Number of records where a new primary record version or a change to "
        "an existing primary record version is backed out due to rollback or "
        "savepoint undo.",
    )
    purges = property(
        __get_purges,
        doc="Number of records where record version chain is being purged of "
        "versions no longer needed by OAT or younger transactions.",
    )
    expunges = property(
        __get_expunges,
        doc="Number of records where record version chain is being deleted due to "
        "deletions by transactions older than OAT.",
    )
    memory_used = property(__get_memory_used, doc="Number of bytes currently in use.")
    memory_allocated = property(
        __get_memory_allocated,
        doc="Number of bytes currently allocated at the OS level.",
    )
    max_memory_used = property(
        __get_max_memory_used, doc="Maximum number of bytes used by this object."
    )
    max_memory_allocated = property(
        __get_max_memory_allocated,
        doc="Maximum number of bytes allocated from the operating system by this object.",
    )
    # FB 3.0
    ...
    backversion_reads = property(
        __get_backversion_reads, doc="Number of record backversion reads."
    )
    fragment_reads = property(
        __get_fragment_reads, doc="Number of record fragment reads."
    )
    repeated_reads = property(
        __get_repeated_reads, doc="Number of repeated record reads."
    )
...
class TableStatsInfo(BaseInfoItem):
    ...
    owner = property(
        __get_owner,
        doc="""Object that owns this TableStats instance. Could be either
        :class:`DatabaseInfo`, :class:`AttachmentInfo`, :class:`TransactionInfo`,
        :class:`StatementInfo` or :class:`CallStackInfo` instance.""",
    )
    ...
    expunges = property(
        __get_expunges,
        doc="Number of records where record version chain is being deleted due to deletions by transactions older than OAT.",
    )
    locks = property(__get_locks, doc="Number of record locks.")
    waits = property(__get_waits, doc="Number of record waits.")
    conflits = property(__get_conflits, doc="Number of record conflits.")
...
class ContextVariableInfo(BaseInfoItem):
    ...
    attachment = property(
        __get_attachment,
        doc=":class:`AttachmentInfo` instance to which this context variable belongs or None.",
    )
    transaction = property(
        __get_transaction,
        doc=":class:`TransactionInfo` instance to which this context variable belongs or None.",
    )
